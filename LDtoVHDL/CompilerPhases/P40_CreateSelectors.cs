...
namespace LDtoVHDL.CompilerPhases
{
	public class P40_CreateSelectors : IPhase
	{
		...
		{
			for (int i = 0; i < env.Rungs.Count; ++i)
			{
				var currentRungBlocks = env.Rungs[i].Blocks;
				var previousRungWritingBlocks = i != 0
					? env.Rungs[i - 1].WritingBlocks
					: new Dictionary<string, List<IOutVariableBlock>>();

				CreateSelectorsInRung(env, currentRungBlocks, previousRungWritingBlocks);
			}
		...
		private static void CreateSelectorsInRung(Environment env, HashSet<BaseBlock> currentRungBlocks, IReadOnlyDictionary<string, List<IOutVariableBlock>> previousRungWritingBlocks)
		{
			...
			foreach (var inVarBlock in currentRungBlocks.OfType<IInVariableBlock>())
			{
				...
				{
					var selector = CreateSelectorForVariable(env, currentRungBlocks, previousRungWritingBlocks, inVarBlock,
						createdSelectors);
					ConnectSelectorToReader(inVarBlock, selector);
				}
				else
					ConnectVariableDirectly(env, inVarBlock);
			}
		}
		...
		private static VarSelector CreateSelectorForVariable(Environment env, HashSet<BaseBlock> currentRungBlocks,
